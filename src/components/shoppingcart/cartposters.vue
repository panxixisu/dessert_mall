<template>
  <div
    class="shoppingcart_wrapper"
    :style="{ backgroundImage: 'url(' + coverData.cover + ')' }"
    ref="shoppingcart_wrapper"
    :class="{
      shoppingcart_wrapper_hide: shoppingcart_wrapper_ishide,
      shoppingcart_wrapper_notHide: !shoppingcart_wrapper_ishide,
    }"
    key="shoppingcart_wrapper"
  >
    <div
      class="title_wrapper"
      ref="cart"
...
export default defineComponent({
  setup() {
    const cart = ref();
    const shoppingcart_wrapper = ref();
    const coverData = {
      categoryid: 4,
      categoryname: "我的购物车",
      cover:
        " http://www.honeymoon-dessert.com/admin/upload/productCategory/banner/42.jpg",
      enname: "ShoppingCart",
      icondata:
        "http://www.honeymoon-dessert.com/admin/upload/productCategory/icon/104.png",
    };
    onMounted(() => {
      setTimeout(() => {
        shoppingcart_wrapper_ishide.value = false;
        ishide.value = false;
      }, 500);
    });
    //监听滚动
    let ishide = ref(true);
    let shoppingcart_wrapper_ishide = ref(true);
    let scrollTop = ref();
    ...
    watch(scrollTop, (newvalue, oldvalue) => {
      ishide.value =
        newvalue > parseInt(cart.value.offsetTop) + 280 ? true : false;
      shoppingcart_wrapper_ishide.value = newvalue > 640 ? true : false;
    });
    return {
      cart,
