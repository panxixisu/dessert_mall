<template>
  ...
  >
   ...
   <el-input class="addressInput" id="updateaddressId" ref="addresss" v-model="addressForm.address" :input-style='{width:"300px"}' placeholder="请输入小区名称"  />
    </el-form-item>
...
let addressForm:any = ref({
  ...
  const dialogShow = computed({
    ...
    get: () => props.isedit,
    set: (val) => {
      context.emit("update:isedit", val);
    },
  });
...
const submitForm = ()=>{
    addressFormRef.value?.validate((v:any) => {
        if (v) {
          if(isEdit.value){
            store.dispatch("UserModule/updateAddress", {
...
const aaa =ref()
    return {dialogShow,addressForm,submitForm,aaa,cancle,rules,addresss,addressFormRef,isEdit}
  }
})
